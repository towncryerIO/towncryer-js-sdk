...
export interface PushNotificationService {
     ...
     */
    getMessageHistory(page?: number, size?: number): Promise<PaginatePage | any>;
...
export class FirebasePushNotificationService implements PushNotificationService {
    ...
    private firebaseMessagingSw: any;
     ...
     */
    getMessageHistory(page: number = 0, size: number = 10): Promise<PaginatePage | any> {
        if (!this.customerId) {
     ...
     * @param token Push notification token
     */
    async registerToken(customerId: string, token: string): Promise<ApiResponse | ApiError> {
        try {
            if (!customerId) {
            ...
            response = await this.eventService.publishEvent(eventPayload);
     ...
     * @private
     */
    private mapFirebaseMessageToNotification(payload: any): PushNotification {
